...
- For any scene dominated by water, sky, meadow, snow, foliage, or ground, run [scripts/audit_surface_uniformity.py](scripts/audit_surface_uniformity.py) on the generated image and a crop containing mostly that material when a local output path is available. Use `--handling allover` only when the source or requested handling genuinely calls for a late pond, dense garden, enveloping fog, or comparable all-over field—not to excuse a failed conventional scene. Treat PASS, REVIEW, and REJECT as diagnostics rather than authenticity judgments; the visual material-crop gates remain mandatory.
...
7. **Design surface rhythm across multiple axes.** For most scenes, divide each dominant material into a quiet body, unequal transitions, and sparse active events. Change spacing, scale, direction, edge, loading, overlap, and burial between them; do not merely jitter individual strokes inside one continuous texture. When the source or requested handling is legitimately all-over—such as a late pond, dense garden, or fog field—density may remain comparatively even. In that mode, keep spacing nonperiodic, mix scales, preserve material-directed flow, and vary at least edge, loading, overlap, burial, color path, or relief. Never force three depth bands into a shallow all-over field.
8. **Route activity as well as marks.** Route marks by material, physical state, depth, light event, and contact. Concentrate paint activity where light changes, overlap, motion, texture, or recognition carry information; keep quieter intervals large enough to read at normal size. Where water meets a bird, boat, rock, bank, or reflection anchor, interrupt and redirect the local surface rhythm with lapping, wake, reflection, occlusion, or wet-contact evidence. Do not place an object on top of an uninterrupted texture field.
9. **Write the prompt in priority order.** State preservation requirements first, requested changes second, broad bodies and activity zones third, sparse surface accents fourth, and exclusions last. Use positive mark nouns sparingly: long lists of dashes, curls, hooks, facets, ripples, or impasto often cause the generator to tile them. State what remains quiet, thin, merged, or buried as explicitly as what becomes active.
10. **Generate and inspect the complete image and material crops.** Check factual preservation before aesthetics, then large masses, form boundaries, activity hierarchy, mark hierarchy, color, and material routing. Inspect the largest dominant-material region by itself in grayscale and color. In ordinarily staged scenes, require visibly different quiet, transitional, and active passages. In a legitimate all-over field, require nonperiodic spacing, mixed scale, material-directed flow, and varied edge or loading even when density stays continuous. Contrast between materials cannot hide a stamped surface.
11. **Retry once when required.** Return to the original source or last factually faithful edit, freeze only passages that have passed every relevant gate, and correct one diagnosed failure. User approval of color, birds, or another local property does not freeze an untested uniform surface. Do not compound drift by repeatedly editing a failed derivative.
12. **Build the companion collage after approval.** Use the original photo and final approved conversion as fixed panels. Choose a source-derived background, one restrained counter-accent, and a subject-related decorative motif; inspect both panels for crop, orientation, and content drift before returning both files.
...
- Treat anti-uniformity as a multi-axis hard requirement, not density modulation alone. Never permit periodic or lattice-like spacing; require mixed scale and material-directed orientation. Most scenes also vary density through calm, transitional, and active passages. A source-supported all-over field may keep density comparatively even only while other axes—scale, spacing, edge, loading, overlap, burial, color path, or relief—remain visibly varied. A canvas fails when one recognizable mark footprint covers a material continuously, even if its colors or angles vary.
...
- Make subjects physically affect adjacent materials. Break, bend, pool, reflect, occlude, or quiet the surrounding stroke field at contact points so rocks, birds, boats, figures, and architecture inhabit the paint rather than sit on top of a wallpaper texture.
...
- The largest water, sky, meadow, snow, foliage, or ground region passes a material-crop test: viewed alone, it has no periodic spacing, one-size stamp, isotropic dot field, or continuous relief footprint. Ordinarily staged scenes show quiet, transitional, and active passages; a legitimate all-over field may keep even density only when scale, flow, edge, loading, overlap, or burial compensates.
- Repeated objects interrupt the surrounding field through state-specific contact evidence—wake, lapping, reflection, cast color, overlap, wet seams, burial, or broken flow—without requiring photographic detail.
...
- The approved clean painting and original photo remain distinct, complete, correctly oriented panels; the collage does not silently repaint, mirror, recrop, or substitute either one. By default the smaller borderless original sits directly on the graphic field above, and the larger converted painting sits below inside a complete museum-style carved frame.
