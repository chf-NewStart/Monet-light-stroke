...
- For any scene dominated by water, sky, meadow, snow, foliage, or ground, run [scripts/audit_surface_uniformity.py](scripts/audit_surface_uniformity.py) on the generated image and a crop containing mostly that material when a local output path is available. Treat warnings as evidence for closer visual inspection, not as automatic proof; the visual material-crop gates remain mandatory.
...
7. **Design spatial-frequency zones.** Before prompting surface marks, divide every dominant material into at least one quiet body, one transitional passage, and one active event. Change mark scale, density, direction, edge, and loading between those zones; do not merely vary individual strokes inside one continuous texture. In conventional depth, make the changes gradual. In horizonless or all-over fields, organize them through irregular islands, overlaps, pauses, and color paths rather than three mechanical bands.
...
10. **Generate and inspect the complete image and material crops.** Check factual preservation before aesthetics, then large masses, form boundaries, activity hierarchy, mark hierarchy, color, and material routing. Inspect the largest dominant-material region by itself in grayscale and color. It must contain visibly different quiet, transitional, and active subzones; contrast between water and foliage or sky and ground cannot hide a uniformly stamped material.
...
- Treat spatial density modulation as a hard requirement. The dominant material must include calm, transitional, and active passages with different scale, density, loading, and edge behavior. A canvas fails when one recognizable mark footprint covers a material continuously, even if its colors and angles vary.
...
- The largest water, sky, meadow, snow, foliage, or ground region passes a material-crop test: viewed alone, it contains at least three perceptibly different activity regimes rather than one tiled frequency or relief level.
