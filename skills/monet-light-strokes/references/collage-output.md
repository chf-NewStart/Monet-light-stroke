...
Use this reference after the clean photo-to-painting conversion has passed its factual and painterly checks. The collage is a second deliverable, not a replacement for the clean painting and not part of the painting prompt.

## Default Deliverables

For each photo transformation, return:

1. the approved clean converted painting at its source-preserving orientation and aspect ratio;
2. a separate portrait collage pairing that exact conversion with the original photo.

The user may opt out of the collage or request another format. Original-scene generation does not require an original-versus-converted collage unless the user asks.

## Source Roles

- The original photo remains the factual authority for its panel.
- The final approved conversion remains the authority for the painting panel. Never build the collage from a rejected first pass.
- A supplied collage example is layout inspiration unless the user explicitly says to reproduce a named element. Do not copy its text, location, date, logos, interface controls, earbuds, waveform, or subject matter by default.
- Treat both content panels as fixed assets. Preserve their complete crop, displayed orientation, aspect ratio, left/right order, and visible subjects. Do not repaint, mirror, zoom, synthesize, or substitute either panel.

## Palette Selection

Choose the graphic palette relationally from the source:
...
## Subject-Derived Decoration

Use one sparse motif related to the source rather than copying arbitrary decoration from a layout reference:

- flowers or plants: a loose vine, stem, petal, or seed path;
- water: a ripple, reflection path, or wave line;
- architecture: a restrained roof, rail, street, or window rhythm;
- figures: a path, gesture echo, or clothing-derived geometric note;
- sky or weather: a cloud edge, wind path, rain trace, or light arc.

Keep decoration in negative space. It must never cover a face, body, focal flower, landmark, or other meaningful panel content.

## Text and UI

Add no title, location, date, caption, lettering, number, logo, signature, audio control, waveform, or photographic UI unless the user supplies or explicitly requests that content. Never infer a location or date from a reference collage.

## Inspection and Retry

Inspect the collage at full view and panel level:
